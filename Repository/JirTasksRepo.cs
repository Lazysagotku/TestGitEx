...
{
    // ✅ Класс для результата объединённого запроса
    internal class JiraCountsResult
    {
    ...
    internal class JiraTasksRepo
    {
        internal static List<int> GetTasksCounts(string displayName)
        {
            ...
            SELECT
                (SELECT COUNT(*)
                 ...
                 WHERE ji.issuestatus NOT IN(10002,10008,10013,10016,5,6)
                   AND cw.display_name = @DisplayName) AS Count1,
                ...
                (SELECT COUNT(*)
                 ...
                 WHERE ji.issuestatus IN (1,10000,10007,10009,10010)
                   AND cw.display_name = @DisplayName) AS Count2,
               
                (SELECT COUNT(*)
                 FROM [JiraSoftwareDB].[dbo].[jiraissue] ji
                 WHERE ji.issuestatus NOT IN(10002,10008,10013,10016,5,6)
                   AND ji.ASSIGNEE IS NULL) AS Count3,
               
                (SELECT ISNULL(FLOOR(SUM(wl.timeworked) / 60.0), 0)
                 ...
                 WHERE cw.display_name = @DisplayName
                   AND CONVERT(date, wl.STARTDATE) = CONVERT(date, GETDATE())) AS Count6
            ";

            // ✅ Явно указываем тип <JiraCountsResult>
            var result = JiraRepository.QuerySingleOrDefault<JiraCountsResult>(
                sql,
            ...
            return new List<int>
            {
                ...
                result?.Count2 ?? 0,
                result?.Count3 ?? 0,
                0,  // Параметр 4 - не используется
                0,  // Параметр 5 - не используется
                result?.Count6 ?? 0
            };
        }
...
";
                        return JiraRepository.Query<FieldsDetailInfo>(q, new { DisplayName = displayName }, "J-05", "Jira").ToArray();
                    }
                case 2:
                    {
                        var q = @"
...
";
                        return JiraRepository.Query<FieldsDetailInfo>(q, new { DisplayName = displayName }, "J-06", "Jira").ToArray();
                    }
                case 3:
                    {
                        var q = @"
...
";
                        return JiraRepository.Query<FieldsDetailInfo>(q, null, "J-07", "Jira").ToArray();
                    }
                case 4:
                case 5:
...
";
                        return JiraRepository.Query<FieldsDetailInfo>(q, new { DisplayName = displayName }, "J-08", "Jira").ToArray();
                    }
                default:
                    return new FieldsDetailInfo[] { };
            }
...
ORDER BY ji.issuenum
";
            return JiraRepository.Query<FieldsIdTasksUserInfo>(q, new { DisplayName = MainForm.UserName, CurDate = curDate }, "J-09", "Jira").ToArray();
        }
...
ORDER BY 1 DESC";
            return JiraRepository.Query<FieldsTimeUserInfo>(q, new { DisplayName = MainForm.UserName }, "J-10", "Jira").ToArray();
        }
...
";
            var rows = JiraRepository.Query<dynamic>(q, new { DisplayName = displayName }, "J-11", "Jira").ToArray();
